...
func (qsem *QSEMEngine) initializeMeaningSpace() error {
	...
	primeBasis := qsem.resonanceEngine.GetPrimeBasis()
	if len(primeBasis) > qsem.config.PrimeBasisSize {
		primeBasis = primeBasis[:qsem.config.PrimeBasisSize]
	}
...
func (qsem *QSEMEngine) generatePrimeBasisForConcept(concept string, index int) []int {
	...
	availablePrimes := qsem.resonanceEngine.GetPrimeBasis()

	// Use available primes with modular selection
	if len(availablePrimes) > 0 {
		// Length-based prime
		lengthPrime := availablePrimes[(conceptLength % len(availablePrimes))]
		primeBasis = append(primeBasis, lengthPrime)

		// Vowel-based prime
		vowelPrime := availablePrimes[(vowelCount % len(availablePrimes))]
		primeBasis = append(primeBasis, vowelPrime)

		// Consonant-based prime
		consonantPrime := availablePrimes[(consonantCount % len(availablePrimes))]
		primeBasis = append(primeBasis, consonantPrime)

		// Hash-based primes
		for i := 0; i < 7 && i < len(availablePrimes); i++ {
			hashPrime := availablePrimes[((hash >> (i * 4)) % len(availablePrimes))]
...
func (qsem *QSEMEngine) generateSemanticAmplitudes(concept string, primeBasis []int) []complex128 {
	...
	for i := 0; i < dimension; i++ {
		var prime float64
		if len(availablePrimes) > 0 {
			prime = float64(availablePrimes[i%len(availablePrimes)])
		} else {
			// Fallback to simple prime calculation
			prime = float64(2 + i) // Simple fallback
		}
...
func (qsem *QSEMEngine) calculateConceptCoherence(quantumState *hilbert.QuantumState, semanticFields map[string]float64) float64 {
	...
	totalCoherence := (quantumCoherence + semanticCoherence) / 2.0

	return totalCoherence
}

// buildConceptualGraph constructs the conceptual relationship graph
func (qsem *QSEMEngine) buildConceptualGraph() error {
	// Placeholder implementation
	return nil
}

// performSemanticLearning executes the semantic learning process
func (qsem *QSEMEngine) performSemanticLearning() (*SemanticAnalysisResult, error) {
	// Create result with basic metrics
	result := &SemanticAnalysisResult{
		ConceptMap:          qsem.semanticVectors,
		ClusterAssignments:  make(map[string]string),
		SemanticSimilarity:  make(map[string]map[string]float64),
		ContextualCoherence: 0.8,
		MeaningDensity:      0.7,
		ConceptualEntropy:   0.5,
		LearningConvergence: 0.9,
