...
func (pm *PerformanceMonitor) Stop() {
	...
	// Use a select to safely close the channel
	select {
...
func (pm *PerformanceMonitor) GetMetric(name string) *PerformanceMetric {
	...
	if metric, exists := pm.metrics[name]; exists {
		...
		// Return a copy
		metricCopy := *metric
		metricCopy.Values = make([]float64, len(metric.Values))
		metricCopy.Timestamps = make([]time.Time, len(metric.Timestamps))
		copy(metricCopy.Values, metric.Values)
		copy(metricCopy.Timestamps, metric.Timestamps)
